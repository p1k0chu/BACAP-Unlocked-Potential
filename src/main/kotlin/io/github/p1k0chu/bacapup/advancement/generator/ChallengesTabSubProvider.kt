...
import io.github.p1k0chu.bacapup.advancement.triggers.SingleEntityTrigger
...
object ChallengesTabSubProvider : AdvancementSubProvider {
    ...
    override fun generate(provider: HolderLookup.Provider, consumer: AdvancementConsumer) {
        advancement(consumer, TAB_NAME, MOB_FLATTENER_9000) {
            ...
            KillMobConstants.getAnvilMobsToKill(provider.lookupOrThrow(Registries.ENTITY_TYPE)).forEach { mob ->
                addCriterion(mob.builtInRegistryHolder().registeredName, BacapupTriggers.ANVIL_KILL.createCriterion(
                    SingleEntityTrigger.Instance(
                        Optional.empty(),
                        listOf(
